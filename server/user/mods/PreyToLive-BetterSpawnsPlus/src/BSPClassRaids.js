...
let BSPClassRaids = class BSPClassRaids {
    ...
    // dynamic
    raidLogs(eftDatabaseLocations, sptConfigsBot, location, logsData) {
        switch (location) {
            case "customs":
    ...
    // dynamic
    raidMaxBots(settingsRaids, bspClassHelpers, sptConfigsBot) {
        if (settingsRaids.maxBotCap.enabled) {
            for (const location of this.locationNames) {
                const randomBotCap = bspClassHelpers.generateRandomInteger(settingsRaids.maxBotCap[location].min, settingsRaids.maxBotCap[location].max);
                switch (location) {
                    case "customs":
    ...
    // dynamic
    raidTimer(settingsRaids, bspClassHelpers, eftDatabaseLocations) {
        if (settingsRaids.raidTimer.enabled) {
            for (const location of this.locationNames) {
                const randomRaidTimer = bspClassHelpers.generateRandomInteger(settingsRaids.raidTimer[location].min, settingsRaids.raidTimer[location].max);
                switch (location) {
                    case "customs":
    ...
    // dynamic
    raidAirdrops(settingsRaids, bspClassHelpers, eftDatabaseLocations, sptConfigsAirdrop) {
    ...
    // static
    raidExtractions(settingsRaids, eftDatabaseLocations) {
    ...
    // static
    raidEnd(settingsRaids, sptConfigsInRaid, sptConfigsLostOnDeath) {
        ...
        sptConfigsLostOnDeath.equipment.Pockets = settingsRaids.lostOnDeath.equipment.Pockets;
    ...
    // static
    raidWeather(settingsRaids, sptConfigsWeather) {
        sptConfigsWeather.acceleration = settingsRaids.weather.dayNightAcceleration;
    }
};
exports.BSPClassRaids = BSPClassRaids;
exports.BSPClassRaids = BSPClassRaids = __decorate([
    (0, tsyringe_1.injectable)(),
    __param(0, (0, tsyringe_1.inject)("WinstonLogger")),
    __metadata("design:paramtypes", [Object])
], BSPClassRaids);
//# sourceMappingURL=BSPClassRaids.js.map